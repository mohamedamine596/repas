...
import { Gift, Package, Heart, MessageCircle, PlusCircle, ArrowRight, ShieldCheck, Search } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: myMeals = [] } = useQuery({
    ...
    queryFn: async () => {
      return backendApi.meals.list({ donor_email: user.email }, "-created_date", 10);
    },
  ...
  const { data: myReservations = [] } = useQuery({
    ...
    queryFn: async () => {
      return backendApi.meals.list({ reserved_by: user.email }, "-created_date", 10);
    },
  ...
  const donorActionLink =
    donorStatus === "email_verified" ? createPageUrl("DonorQuiz") : createPageUrl("Profile");
      ...
      {isReceiver && (
        <Card className="border-emerald-200 bg-emerald-50">
          <CardContent className="p-4 text-sm text-emerald-900">
            En tant que receveur, vous pouvez reserver des repas immediatement apres inscription.
          </CardContent>
        </Card>
      )}
          ...
          >
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-[#51627a]">{stat.title}</p>
                  <p className="text-2xl font-bold text-[#0f2a57] mt-1">{stat.value}</p>
                </div>
                <div className={`w-10 h-10 rounded-xl border border-[#dbe5f2] bg-white ${stat.bgColor} flex items-center justify-center`}>
          ...
          {myMeals.length === 0 ? (
            <Card className="border-[#f0e8df] border-dashed">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-500">Vous n'avez pas encore publie de repas.</p>
